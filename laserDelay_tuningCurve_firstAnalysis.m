function[masterData, slaveData, repPsthData] = laserDelay_tuningCurve_firstAnalysis(path,speed)
...
if speed == 'fast'
    ...
    for f=1:length(unique( outer_stim_seq_vector))
        ...
        for j=1:length(values_inner)
            ...
            fra_std(j,f)=std(fr)/sqrt(length(fr));
    ...
    masterStdData{chanCounter} = fra_std;
...
%                 raster_t_slave(:,i) = tmpRaster_slave;
        ...
        fra_std_slave(j,f)=std(fr_slave)/sqrt(length(fr_slave));
...
slaveStdData{chanCounter} = fra_std_slave;
chanCounter = chanCounter +1;
...
% startingFrequency = 4000;
% for i = outerSeq.master.iorder
%     frequencySteps(i) = (startingFrequency*((2^i)*.3))
% end
...
% clear subplot
% % Plot a heatmap for each channel, either for master or slave
% clims = [minRate(1),maxRate(1)];
% figure
% j =1;
% for i = [9,8,10,7,13,4,12,5,15,2,16,1,14,3,11,6]
%     subplot(8,2,j)
%     j=j+1
%     imagesc(outerSeq.master.values./1000,innerSeq.master.values,masterData{i},clims)
...
% colormap(hot)
% 
% % Plot heatmaps of master and slave for a particular channel on the same
% % figure
% clims = [minRate(1),maxRate(1)];
% figure
% subplot(2,1,1)
% imagesc(outerSeq.master.values./1000,innerSeq.master.values,slaveData{4},clims)
% axis xy
% colormap(hot)
% title('Tone')
% subplot(2,1,2)
% imagesc(outerSeq.master.values./1000,innerSeq.master.values,masterData{4},clims)
% axis xy
% colormap(hot)
% title('Tone + Laser')
% 
...
%     subplot(8,2,j)
%     j=j+1
%     plot(outerSeq.master.values./1000,diffData{i}')
%     %title(['Channel #' num2str(i)])
%     set(gca,'box','off')
% end
...
for i = [7,13,4,12,5,15,2,16,1,14,3,11,6]
    ...
    %     plot(diffData{i}')
    %     set(gca,'box','off')
    %     axis([1 8 min(min(diffData{i})) max(max(diffData{i}))])
    %     j = j+1;
...
xlabel('Frequency (KHz)')
 ...
 % Plot each channel as a graph, with each delay as a subplot +/- laser with
 % error bars
